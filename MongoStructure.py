...
parser.add_argument('-database', default='school', help='mongoDB Database name')
parser.add_argument('-collection', default='null', help='mongoDB Collection name')
...
# 所有表
def create_structure_item(host, port, database, collection, out_type,table):
    try:
        total = table.estimated_document_count()
        if total>1000:
            total = 1000;
        ...
        for it in tqdm(table.find(no_cursor_timeout=True).sort([('_id', -1)]).limit(total), total=total, desc="Collection Structure"):
            __node_structure(it, collection_structure)
        print('Done!')
        print('=' * 10, database + '.' + collection, '=' * 10)
        if out_type == 'tree':
...
def find_all(host,port,database,out_type):
    client = MongoClient(host, port)
    db = client[database]
    coll_names = db.list_collection_names(session=None)
    for c in coll_names:
        table = client[database][c]
        create_structure_item(host,port,database,c,out_type,table)
    client.close()

if __name__ == '__main__':
    args = parser.parse_args()
    if args.collection == 'null':
        find_all(args.host, args.port, args.database, args.out_type)
    else:
